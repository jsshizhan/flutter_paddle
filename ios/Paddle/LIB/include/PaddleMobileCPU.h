...
typedef enum EPaddleColorFormat {
    EPaddleColorFormatRGB = 0, // default
    EPaddleColorFormatBGR = 1 // form caffe
};
...
- (PaddleMobileCPUResult *)predict:(CGImageRef)image dim:(NSArray<NSNumber *> *)dim means:(NSArray<NSNumber *> *)means scale:(float)scale format:(EPaddleColorFormat)format;
